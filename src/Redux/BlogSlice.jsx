...
// -----------------------------------------------------------------------------------
// It is used where token is required, through this we are passing tokens
...
export const fetchMyBlog = createAsyncThunk("blogs/fetchMyBlog", async () => {
  const response = await axios.get("/getMyBlog", config);
  return response.data;
});
...
export const createBlogPost = createAsyncThunk(
  ...
  async (newPost) => {
    const response = await axios.post("/createblog", newPost, config);
    return response.data;
  }
);
...
const BlogSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchMyBlog.pending, (state) => {
        state.status = "loading";
      })
      .addCase(fetchMyBlog.fulfilled, (state, action) => {
        state.status = "succeeded";
        state.posts = action.payload;
        state.error = null;
      })
      .addCase(fetchMyBlog.rejected, (state, action) => {
        state.status = "failed";
        state.error = action.error.message;
      })
